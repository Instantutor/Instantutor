...
router.put("/edit/:request_id", auth, async (req, res) => {
    ...
    try {
      await Request.findOne({user: req.user.id})
        .then(doc => {
...
router.put(
  "/bid",
  [auth,
  [check("peer_id", "Bid must include the ID of the request maker").not().isEmpty(),
  check("request_id", "Bid must include the ID of the request you are bidding for").not().isEmpty()]],
  ...
  async (req, res) => {
    ...
    if (peer_id === req.user.id)
      return res.status(403).json({error: "User can not bid on their own request"});
    
    try {
      await Request.findOne({ user: peer_id})
        .then(doc => {
          let requestMatch = doc.requests.id(request_id);

          if (requestMatch) {
            let bidMatch = requestMatch.bids.find(bid => bid.bidder_id === req.user.id);

            if (bidMatch)
              for (const prop in bidFields)
                bidMatch[prop] = bidFields[prop]
            else
              requestMatch.bids.push({bidder_id: req.user.id, ...bidFields});

            doc.save();
            res.json({msg: "Bid made", bid: {bidder_id: req.user.id, ...bidFields}});
          } else {
            res.status(400).json({error: "Request ID is invalid"});
          }
        });
...
router.delete(
  ...
  check("request_id", "ID of bid up for deltion must be in the body").not().isEmpty()]],
      ...
      await Request.findOne({ user: peer_id})
        .then(doc => {
          ...
          if (requestMatch) {
            ...
            if (bidMatch)
              requestMatch.bids = requestMatch.bids.filter(bid => 
                bid.bidder_id.toString() !== req.user.id
              );
            else
              res.status(403).json({error: "You have not placed any bids on this request"});
