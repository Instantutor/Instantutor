export const SET_ALERT = "SET_ALERT";
export const REMOVE_ALERT = "REMOVE_ALERT";
export const REGISTER_SUCCESS = "REGISTER_SUCCESS";
export const REGISTER_FAIL = "REGISTER_FAIL";
export const USER_LOADED = "USER_LOADED";
export const AUTH_ERROR = "AUTH_ERROR";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAIL = "LOGIN_FAIL";
export const LOGOUT = "LOGOUT";

//profile
export const GET_PROFILE = "GET_PROFILE";
export const UPDATE_PROFILE = "UPDATE_PROFILE";
export const CLEAR_PROFILE = "CLEAR_PROFILE";
export const PROFILE_ERROR = "PROFILE_ERROR";
export const ACCOUNT_DELETED = "ACCOUNT_DELETED ";
export const GET_PEER_PROFILE = "GET_PEER_PROFILE";
export const GET_TUTORS = "GET_TUTORS";

//calendar
export const CONFIRM_CALENDAR_CREATION = "CONFIRM_CALENDAR_CREATION";
export const CREATE_CALENDAR = "CREATE_CALENDAR";
export const DELETE_CALENDAR = "DELETE_CALENDAR";
export const GET_WEEK_EVENTS = "GET_WEEK_EVENTS";
export const CLEAN_CALENDAR = "CLEAN_CALENDAR";
export const CREATE_EVENT = "CREATE_EVENT";
export const EDIT_EVENT = "EDIT_EVENT";
export const DELETE_EVENT = "DELETE_EVENT";
export const CALENDAR_ERROR = "CALENDAR_ERROR";

//search
export const GET_SEARCH = "GET_SEARCH";
export const SEARCH_ERROR = "SEARCH_ERROR";
export const CLEAR_SEARCH = "CLEAR_SEARCH";
export const AUTO_SUGGESTION = "AUTO_SUGGESTION";

//user request
export const POST_USER_REQUEST = "POST_USER_REQUEST";
export const GET_USER_REQUEST = "GET_USER_REQUEST";
export const EDIT_USER_REQUEST = "EDIT_USER_REQUEST";
export const DELETE_USER_REQUEST = "DELETE_USER_REQUEST";
export const USER_REQUEST_ERROR = "USER_REQUEST_ERROR";
export const CLEAR_USER_REQUEST = "CLEAR_USER_REQUEST";
export const GET_CONFIRMED_TUTORS = "GET_CONFIRMED_TUTORS";
export const GET_CONFIRMED_TUTORS_ERROR = "GET_CONFIRMED_TUTORS_ERROR";
export const CANCEL_REQUEST = "CANCEL_REQUEST";
export const CANCEL_REQUEST_ERROR = "CANCEL_REQUEST_ERROR";
export const CLOSE_REQUEST = "CLOSE_REQUEST";
export const CLOSE_REQUEST_ERROR = "CLOSE_REQUEST_ERROR";
export const RATE_USER_REQUEST = "RATE_USER_REQUEST";
export const RATE_REQUEST_ERROR = "RATE_REQUEST_ERROR";

//peer request
export const CHECK_NEW_PEER_REQUEST = "CHECK_NEW_PEER_REQUEST";
export const PEER_REQUEST_ERROR = "PEER_REQUEST_ERROR";
export const UPDATE_CHECK_TIME = "UPDATE_CHECK_TIME";
export const REQUEST_RESPONSE = "REQUEST_RESPONSE";
export const RATE_PEER_REQUEST = "RATE_PEER_REQUEST";
export const GET_OPEN_PEER_REQUESTS = "GET_OPEN_REQUESTS";
export const ADD_TO_PEER_REQUEST = "ADD_TO_PEER_REQUEST";

//disperse requests
export const DISPERSE_REQUESTS = "DISPERSE_REQUESTS";
export const DISPERSE_FINAL_REQUEST = "DISPERSE_FINAL_REQUEST";
export const DISPERSE_REQUEST_ERROR = "DISPERSE_REQUEST_ERROR";
