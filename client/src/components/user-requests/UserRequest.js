...
const UserRequest = ({
  ...
  useEffect(async () => {
    if (requestID && user) {
      if (request_history.length === 0)
        await getRequestHistory(user._id).then(
          console.log('success: ', request_history)
        )
      ...
      for (const prop in requestData)
        if (request && request[prop]) oldRequestData[prop] = request[prop]
  ...
  const onSubmit = async (e) => {
    e.preventDefault()
    if (requestID) await editRequest(requestData, requestID, history)
    else await createRequest(requestData, history)
  }
  ...
  const checkbox = day_times.map((time) => (
    <tr key={time}>
      ...
      {day_names.map((day) =>
        availability &&
        availability.find((period) => period === day + ' ' + time) ? (
          <td key={day}>
            <input
              className='checkbox'
              name={day + ' ' + time}
            ...
            <div className='form-group'>
              <select name='course' value={course} onChange={onChange}>
                ...
                {subject in courses.course_list
                  ? courses.course_list[subject].map((course) => (
                      <option value={course}>{course}</option>
                    ))
                  : null}
              </select>
              <small className='form-text'>
                What course do you need help with eg. Data Structures, Organic
                Chemistry, Linear Algebra ...?
              </small>
            </div>
