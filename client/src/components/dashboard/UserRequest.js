...
const UserRequest = ({ user_request, deleteRequest }) => {
  const user_request_util = removeReqDups(user_request);
  const requests = user_request_util.map((req) => (
      ...
      {req.status === "open" && (
        <tr>
          ...
          <td className="hide-sm">
            {new Date(req.last_edit_time).toLocaleDateString()}
          </td>

          <td>
            <Link
              to={`/request_matched_tutors/${req._id}`}
