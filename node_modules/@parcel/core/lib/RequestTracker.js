...
class RequestGraph extends _ContentGraph().default {
  constructor(...args) {
    super(...args);
  ...
  replaceSubrequests(requestNodeId, subrequestContentKeys) {
    ...
    this.replaceNodeIdsConnectedTo(requestNodeId, subrequestNodeIds, null, 'subrequest');
  }
  ...
  invalidateEnvNodes(env) {
    for (let nodeId of this.envNodeIds) {
      ...
      if (env[node.value.key] !== node.value.value) {
        let parentNodes = this.getNodeIdsConnectedTo(nodeId, 'invalidated_by_update');
  ...
  invalidateOnFileCreate(requestNodeId, input) {
    ...
    if (input.glob != null) {
      ...
      for (let part of parts) {
        ...
        if (lastNodeId != null && !this.hasEdge(lastNodeId, fileNameNodeId, 'dirname')) {
  ...
  getInvalidations(requestNodeId) {
    ...
    let invalidations = this.getNodeIdsConnectedFrom(requestNodeId, 'invalidated_by_update');
  ...
  invalidateFileNameNode(node, filePath, matchNodes) {
    ...
    for (let matchNode of matchNodes) {
      ...
      if (this.hasEdge(nodeId, matchNodeId, 'invalidated_by_create_above') && (0, _utils().isDirectoryInside)(_path().default.dirname(matchNode.value.filePath), dirname)) {
        let connectedNodes = this.getNodeIdsConnectedTo(matchNodeId, 'invalidated_by_create');
    ...
    if (this.hasContentKey(contentKey)) {
      if (this.hasEdge(nodeId, this.getNodeIdByContentKey(contentKey), 'dirname')) {
  ...
  respondToFSEvents(events) {
    ...
    for (let {
      ...
      let hasFileRequest = this.hasContentKey(filePath); // sometimes mac os reports update events as create events.
      ...
      if (hasFileRequest && (type === 'create' || type === 'update')) {
        ...
        if (type === 'create') {
          let nodes = this.getNodeIdsConnectedTo(nodeId, 'invalidated_by_create');
        ...
        if (fileNameNode != null && (fileNameNode === null || fileNameNode === void 0 ? void 0 : fileNameNode.type) === 'file_name') {
          ...
          let above = this.getNodeIdsConnectedTo(fileNameNodeId, 'invalidated_by_create_above').map(nodeId => {
            let node = (0, _nullthrows().default)(this.getNode(nodeId));
        ...
        for (let connectedNode of this.getNodeIdsConnectedTo(nodeId, 'invalidated_by_delete')) {
          didInvalidate = true;
...
class RequestTracker {
  ...
  async writeToCache() {
    let cacheKey = `${_constants().PARCEL_VERSION}:${JSON.stringify(this.options.entries)}`;
...
async function loadRequestGraph(options) {
  ...
  let cacheKey = `${_constants().PARCEL_VERSION}:${JSON.stringify(options.entries)}`;
  let requestGraphKey = (0, _hash().hashString)(`${cacheKey}:requestGraph`);
