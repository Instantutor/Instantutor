...
var _default = new (_plugin().Runtime)({
  apply({
    ...
    if (options.shouldBuildLazily && bundle.env.outputFormat === 'global') {
      let referenced = bundleGraph.getReferencedBundles(bundle).filter(b => b.bundleBehavior !== 'inline');
...
function getLoaderRuntime({
  ...
  let externalBundles = bundleGraph.getBundlesInBundleGroup(bundleGroup).filter(bundle => bundle.bundleBehavior !== 'inline');
...
function shouldUseRuntimeManifest(bundle, options) {
  let env = bundle.env;
  return !env.isLibrary && bundle.bundleBehavior !== 'inline' && env.outputFormat === 'global' && env.isBrowser() && options.mode === 'production';
}