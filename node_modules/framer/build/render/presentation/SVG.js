...
class SVGComponent extends Layer {
    ...
    identifier = "lsvg" + Math.round(Math.random() * 100000);
    render() {
        if (process.env.NODE_ENV !== "production" && safeWindow["perf"])
            safeWindow["perf"].nodeRender();
        const { id, visible, style, fill, svg, intrinsicHeight, intrinsicWidth, layoutId, className, variants, transition, withExternalLayout, innerRef, svgContentId, } = this.props;
        ...
        const aria = ariaElements(this.props, this.identifier);
        return (React.createElement(motion.div, { ...dataProps, layoutId: layoutId, transformTemplate: hasTransformTemplate ? transformTemplate(this.props.center) : undefined, id: id, ref: innerRef, style: outerStyle, className: className, variants: variants, transition: transition, tabIndex: this.props.tabIndex, role: aria ? "img" : undefined, "aria-labelledby": aria ? ariaLabelledBy(this.props, this.identifier) : undefined },
            content,
            aria));
    }
}
function ariaElements(props, id) {
    if (!props.title && !props.description)
        return null;
    return (React.createElement(React.Fragment, null,
        props.title && (React.createElement("div", { style: VISUALLY_HIDDEN_STYLES, id: id + "-aria-title" }, props.title)),
        props.description && (React.createElement("div", { style: VISUALLY_HIDDEN_STYLES, id: id + "-aria-desc" }, props.description))));
}
function ariaLabelledBy(props, id) {
    let s = "";
    if (props.title) {
        s += id + "-aria-title";
    }
